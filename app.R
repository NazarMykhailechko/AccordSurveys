...
option = option <- c(
 ...
 "Напишіть коментар"),
  ...
  input_type = c(rep("matrix", 44),rep("text", 1)),
  input_id = c(rep("q11", 22),rep("q12", 22),rep("comment", 1)),
  dependence = NA,
  dependence_value = NA,
  required = c(rep(TRUE, 44),rep(FALSE, 1))
...
"))
  ...
  div( id = "display_survey",
       ...
       shinysurveys::surveyOutput(df = read.csv2(file = "ddd.csv",header = TRUE, sep = ";"),
